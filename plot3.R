...
plot3 <- function() {
  ...
  plot(strptime(dt$Datetime, "%d/%m/%Y %H:%M:%S"), dt$Sub_metering_1, type = "l", xlab = "", ylab = "Energy sub metering")
  lines(strptime(dt$Datetime, "%d/%m/%Y %H:%M:%S"), dt$Sub_metering_2, type = "l", col = "red" )
  lines(strptime(dt$Datetime, "%d/%m/%Y %H:%M:%S"), dt$Sub_metering_3, type = "l", col = "blue" )
